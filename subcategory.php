...
function CreateCategory(){
    //echo $subcatid;
    $subcatid=$_GET["subcatid"];
     $result=  mysql_query("select categoryid,maincategoryid, categoryname from categories where maincategoryid=$subcatid;") or die(mysql_error());
 
$maincategorylink="<ul id=\"maincategorieslink\" >";
while($row = mysql_fetch_assoc($result)) {

 // $dropdown .= "\r\n<option value='{$row['maincategoryid']}'>{$row['maincategoryname']}</option>";
$maincategorylink.="<li><a href=\"questionpage.php?subcatid={$row['categoryid']}\">{$row['categoryname']}</a> </li>";
}
